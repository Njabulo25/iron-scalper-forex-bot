...
class GoldScalper:
    ...
    def get_asian_range(self, day_start):
        ...
        return asian_data["high"].max(), asian_data["low"].min(), len(asian_data)
    ...
    def generate_signals(self):
        ...
        for i in range(20, len(self.df)):
            ...
            if result is None:
                continue
            asian_high, asian_low, asian_bars = result
            ...
            sl_distance = asian_range * 0.5
            
            if close > asian_high:
                if close - asian_high > 1.0 * atr5:
                    continue
                if rsi < 45:
                    continue
                ...
                trades.append({
                    ...
                    "time": curr_time, "asian_high": asian_high,
                    "asian_low": asian_low, "asian_range": asian_range
                })
                days_processed.add(day_start)
            
            elif close < asian_low:
                if asian_low - close > 1.0 * atr5:
                    continue
                if rsi > 55:
                    continue
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("DAILY BREAKOUT STRATEGY")
    print("=" * 70)
    print("Signals:", len(trades))
    for t in trades:
        print(f"  {t['time']} | {t['type']} | Entry: {t['entry']:.2f} | SL: {t['sl']:.2f} | TP: {t['tp']:.2f} | Range: {t['asian_range']:.2f}")
'''
